...
void Camera::fitInView(float xMin, float xMax, float yMin, float yMax)
{
	...
	float screenAspect = this->aspect;
	...
	if (screenAspect < ratio) {
		...
		left = xMin;
		right = xMax;
		top = yMax + fabs( w / screenAspect - h) / 2.0f;
		bottom = yMin - fabs(w / screenAspect - h) / 2.0f;
	} else {
		//printf("screen > \n");
		left = xMin - fabs(h * screenAspect - w) / 2.0f;
		right = xMax + fabs(h * screenAspect - w) / 2.0f;
		top = yMax;
		bottom = yMin;
	}

	position = {(xMin + xMax) * 0.5, (yMin + yMax) * .5, 1};
	zNear = -10;
	zFar = 10;
}
